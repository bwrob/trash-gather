/**
 * @file test_object.c
 * @brief Unit tests for object field properties, array bounds/mutation, and
...
munit_case(RUN, test_array_set_outside_bounds, {
  ...
  assert(array_set(obj, 1, outside));
  assert_false(array_set(obj, 2, outside));
  assert_false(array_set(obj, 100, outside));
...
munit_case(SUBMIT, test_array_set_rejects_invalid_inputs, {
  ...
  object_t *not_array = new_integer(5);

  assert_false(array_set(NULL, 0, value));
  assert_false(array_set(array, 0, NULL));
  assert_false(array_set(not_array, 0, value));
...
munit_case(RUN, test_array_get, {
  ...
  object_t *retrieved_first = array_get(obj, 0);
  ...
  object_t *retrieved_second = array_get(obj, 1);
...
munit_case(RUN, test_array_get_empty_slot, {
  ...
  assert_null(array_get(obj, 1), "Empty array slots should be NULL");
...
munit_case(SUBMIT, test_array_get_outside_bounds, {
  ...
  assert_null(array_get(obj, 1), "Should not access outside the array");
...
munit_case(SUBMIT, test_array_get_rejects_invalid_inputs, {
  ...
  assert_null(array_get(NULL, 0), "Should reject NULL input");
  assert_null(array_get(not_array, 0), "Should reject non-array input");
...
munit_case(RUN, test_add_arrays, {
  vm_new();
  object_t *arr1 = new_array(2);
  object_t *elem1 = new_integer(10);
  object_t *elem2 = new_integer(20);
  array_set(arr1, 0, elem1);
  array_set(arr1, 1, elem2);

  object_t *arr2 = new_array(1);
  object_t *elem3 = new_integer(30);
  array_set(arr2, 0, elem3);
  ...
  assert_int(array_get(res, 0)->data.v_int, ==, 10);
  assert_int(array_get(res, 1)->data.v_int, ==, 20);
  assert_int(array_get(res, 2)->data.v_int, ==, 30);
...
munit_case(RUN, test_add_invalid_mismatched, {
  ...
  object_t *v = new_vector3(i, i, i);
  object_t *a = new_array(1);
