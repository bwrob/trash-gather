...
1. Any function over 40 lines, or nested past depth 2? Split it.
1. Any contract comment containing "and"? Split the function.
1. Any `goto` the repo does not already sanction? Decompose, or justify at the label.
1. Every fallible call checked and propagated?
1. Prototypes at the top of the file, matching every definition?
1. Each new error value: how many producers? Reduce toward one.
1. Any logic pasted twice? Extract it.
1. Any parameter list past 4? Struct it.
1. Header exposes only what callers need?
1. Any function mixing helper calls with inline logic? Push the logic into a leaf.
1. Any helper whose name paraphrases its body? Inline it.
1. Parameters out of context, outputs, inputs order? Reorder.
1. Any loop driven by external input without a named bound, or a nonterminating loop without a marker? Bound or mark it.
1. Any recursion reachable from external input? Convert to a bounded worklist.
1. Any state-mutating leaf with zero asserts? State the invariant.
1. Any pointer parameter not written through that lacks `const`?
...
1. **The branches encode data as control flow.** Cell type to score is a mapping, not logic. A mapping belongs in one lookup leaf, where the next cell type costs one line instead of one pasted block.
1. **Three concepts interleave in one body**: deciding edibility, awarding score, and mutating the map. No single question about this function has a single home.
1. **`map_cell_t cell;` sits uninitialized above the guards.** Declare at the point where the first valid value exists.
