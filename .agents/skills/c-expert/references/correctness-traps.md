...
1. **Hand-roll** integer and fraction emission with `.` hardcoded. Also removes the `printf` parse from a hot path.
...
1. Compare the count, as an unsigned value, against the remaining capacity after the validated start.
1. Only then compute the end coordinate, now proven in range.
