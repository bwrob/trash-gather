...
2. **Isolate Measurements**: Use `state.PauseTiming()` and `state.ResumeTiming()` to ensure setup/teardown costs do not pollute the measured operation.
3. **Realistic Workloads**: Design benchmarks reflecting real-world access patterns (transient churn, deep trees, cyclic clusters, varying retention ratios).
4. **Zero Compiler Tricks**: Prevent dead-code elimination using `benchmark::DoNotOptimize`.
