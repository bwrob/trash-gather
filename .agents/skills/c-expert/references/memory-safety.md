...
## SEI CERT C Standards & Defensive Systems Patterns

Incorporate the formal rules from the **SEI CERT C Coding Standard**:

- **MEM30-C (No Use-After-Free)**: Never dereference a pointer after passing it to `free()`. Set pointers to `NULL` immediately after deallocation in persistent container slots.
- **MEM31-C (No Memory Leaks)**: Every dynamic allocation must have a deterministic deallocation site. In multi-resource functions, unwind partial allocations on failure branches (`goto cleanup`).
- **MEM33-C (Flexible Array Member Sizing)**: Allocate structures containing flexible array members using dynamic allocation with explicitly computed byte counts: `malloc(sizeof(struct s) + n * sizeof(elem))`. Never allocate them statically or embed them by value in unions.
- **MEM34-C (Only Free Dynamic Memory)**: Never invoke `free()` on pointers pointing to the stack, static storage, or internal struct member offsets.
- **MEM35-C (Allocate Sufficient Memory)**: Prefer `sizeof(*ptr)` over `sizeof(type)` to eliminate type-mismatch vulnerabilities when refactoring.
- **INT30-C / INT32-C (Allocation Overflow Protection)**: Before computing `n * sizeof(elem)`, check `if (elem_size != 0 && n > SIZE_MAX / elem_size)` to prevent 32-bit/64-bit integer wraparound.

### SQLite Defensive Engineering Principle

- **`assert()` verifies logic, not runtime fallibility**: Use `assert()` exclusively for internal invariants that *must* hold if code is bug-free (e.g. `assert(obj != NULL)` in private helpers).
- **Never use `assert(ptr != NULL)` after `malloc()`**: Out-of-memory (OOM) is a normal runtime condition. An allocator failure must bubble up gracefully, releasing partially allocated resources.
...
1. Validate any integer used as a size, index, or count against its real domain, before use.
1. Bound total allocation for one message, not just each field. A million valid 1KB fields is still a memory exhaustion.
1. Bound nesting depth explicitly, with a named constant.
